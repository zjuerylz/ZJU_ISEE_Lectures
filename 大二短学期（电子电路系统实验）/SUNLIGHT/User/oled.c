/* USER CODE BEGIN SUNLIGHT_R12_OLED_SOURCE */
/**
 * @file oled.c
...
// OLED器件地址
#define OLED_DEFAULT_ADDRESS 0x78U
...
static uint16_t oled_address = OLED_DEFAULT_ADDRESS;
 ...
 */
static HAL_StatusTypeDef OLED_Send(const uint8_t *data, uint16_t len)
{
  return HAL_I2C_Master_Transmit(&hi2c2, oled_address, (uint8_t *)data, len,
                                 OLED_I2C_TIMEOUT_MS);
}
...
{
  uint8_t sendBuffer[2] = {0U, 0U};
  sendBuffer[1] = cmd;
  return OLED_Send(sendBuffer, sizeof(sendBuffer));
}
 ...
 */
void OLED_SetAddress(uint16_t address)
{
...
HAL_StatusTypeDef OLED_Init(void)
{
  ...
  status = OLED_Send(initSequence, sizeof(initSequence));
  if (status != HAL_OK)
  {
    return status;
  }
...
{
  HAL_StatusTypeDef status;
  uint8_t commandBuffer[4] = {0x00U, 0xB0U, 0x00U, 0x10U};
  static uint8_t sendBuffer[OLED_COLUMN + 1];
  sendBuffer[0] = 0x40U;
  for (uint8_t i = 0; i < OLED_PAGE; i++)
  {
    commandBuffer[1] = (uint8_t)(0xB0U + i);
    status = OLED_Send(commandBuffer, sizeof(commandBuffer));
    if (status != HAL_OK)
    {
      return status;
    }
    memcpy(sendBuffer + 1, OLED_GRAM[i], OLED_COLUMN);
    status = OLED_Send(sendBuffer, sizeof(sendBuffer));
    if (status != HAL_OK)
    {
      return status;
    }
...
/* USER CODE END SUNLIGHT_R12_OLED_SOURCE */
